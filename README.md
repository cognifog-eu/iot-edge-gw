...
(Further information on **Managing the STAs associated to the hotspot** can be found in https://github.com/cognifog-eu/iot-edge-gw/blob/main/managing_STAs.md)
...
The IoT edge GW can be integrated with an external monitoring manager (e.g., Prometheus, with a potential extension to Thanos for long-term storage and scalability) through the installation of a monitoring agent. Even if the IoT edge GW has been already integrated with an external OCM manager, this installation must be conducted MANUALLY by following the steps of the [COGNIFOG's monitoring agent repository](https://github.com/cognifog-eu/monitoring-agent/) 
